        ...
        if( time_point_open_warning_ != 0 )
        {
			if( iter->time_stamp - time_point_open_warning_  >= para_.continue_second )
            {
				...
				this->app_->local_logger().LogLocal(TagOfOrderLog(), 
					TSystem::utility::FormatStr("触发任务:%d 破位卖出 %s 价格:%f 数量:%d | 阀值:%d 秒", para_.id, this->code_data(), price, para_.quantity, time_span)); 
                this->app_->AppendLog2Ui("触发任务:%d 破位卖出 %s 价格:%f 数量:%d | 阀值:%d 秒", para_.id, this->code_data(), price, para_.quantity, time_span); 
                ...
                {
                    auto str = new std::string(utility::FormatStr("执行任务:%d 破位卖出 %s %.2f %d 成功!", para_.id, para_.stock.c_str(), price, para_.quantity));
