...
int _tmain(int argc, _TCHAR* argv[])
{
 ...
 //       //以下是高级批量版功能函数
	...
	int ClientID = Logon("122.224.113.121", 7708, "2.20", 1, "32506627", "32506627", "626261", "", ErrInfo);
	//登录第二个帐号
	...
	else
	{
        ...
        cout << "查询资金结果:"<<Result << " " << ErrInfo << std::endl;

        char result1[1024*5] = {0};
        char result2[1024*5] = {0};
        char * results[] = {result1, result2};
        char erro1[1024] = {0};
        char erro2[1024] = {0};
        char * erros[] = {erro1, erro2};

        int cat_array[] = {3};
        char result[10*1024] = {0};
        QueryDatas(ClientID, cat_array, 1, results, erros);  //ok
        cout << results[0] << "\n" << erros[0] << std::endl;
        goto OUT_PROC;
        //PriceType表示报价方式 0上海限价委托 深圳限价委托 1(市价委托)深圳对方最优价格  2(市价委托)深圳本方最优价格  3(市价委托)深圳即时成交剩余撤销  4(市价委托)上海五档即成剩撤 深圳五档即成剩撤 5(市价委托)深圳全额成交或撤销 6(市价委托)上海五档即成转限价
        // SendOrder(int ClientID, int buy_sell ,int PriceType,
        //"601988"
		//SendOrder(ClientID, 0, 0, "A823751013", "600104", 31.40f, 100, Result, ErrInfo);//买入股票
#if 0
		SendOrder(ClientID, 0, 0, "0171016756", "002342", 10.02f, 100, Result, ErrInfo);//买入股票
        ...
        int catgorys[] = {0, 0};
        int priceTypes[] = {0, 0};
        char* gddms[] ={"0171016756", "A823751013"};
        char* stock_codes[] = {"002342", "601318"};
        float prices[] = {9.15, 45.53};
        int qtys[] = {100, 100};
       

        SendOrders(ClientID, catgorys, priceTypes, gddms, stock_codes, prices, qtys, 2, results, erros);
        for( int i=0; i<2; i++ )
        {
            cout << "买入下单结果:" << Result[i] << " " << ErrInfo[i] << endl;
        }
