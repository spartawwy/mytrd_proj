...
Agent_HUA_TAI::Agent_HUA_TAI()
{  
	strcpy_s(dll_pre_str_, sizeof(dll_pre_str_), "trd_hua_tai");
}
...
bool Agent_HUA_TAI::Setup(char* account_no)
{
	return __Setup(account_no); 
}


bool Agent_HUA_TAI::Login(char* ip, short port, char* ver, short yybid, char* account_no
...
bool Agent_HUA_TAI::InstallAccountData(char *error)
{
	assert(error);
...
int Agent_HUA_TAI::QueryPosition(T_PositionData *out_pos_data, int max_pos_size, char* error)
{
    assert(trade_delegater_);
    if( trade_client_id_ == -1 )
        return -1;
...
bool Agent_HUA_TAI::QueryCapital(T_Capital *capital)
{
    assert(capital);
